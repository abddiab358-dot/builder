...
export function useTasks(projectId?: string) {
  ...
  const createTask = async (input: Omit<Task, 'id' | 'createdAt'>) => {
    ...
    const promises = [log({ action: 'إنشاء مهمة', entity: 'task', entityId: id, details: input.title })]
  ...
  const updateTask = async (id: string, patch: Partial<Task>) => {
    ...
    log({ action: 'تحديث مهمة', entity: 'task', entityId: id }).catch(() => {})
  }

  const deleteTask = async (id: string) => {
    await collection.save((items) => items.filter((t) => t.id !== id))
    log({ action: 'حذف مهمة', entity: 'task', entityId: id }).catch(() => {})
  }
